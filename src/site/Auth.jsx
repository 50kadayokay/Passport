...
import { Loader2, Mail, Lock, AlertTriangle, CheckCircle2, ArrowLeft } from "lucide-react";
...
export default function Auth({ mode }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (isSignup) {
        // Public signup is investor-only; companies are provisioned by an admin.
        const { session, needsConfirmation } = await signUp(email.trim(), password, { role: "investor" });
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100dvh-140px)] max-w-md flex-col justify-center px-6 py-16">
      ...
      <p className="mt-1.5 text-[15px] text-slate-500">{isSignup ? "Discover and follow junior mining companies." : "Sign in to your Passport account."}</p>
      ...
      {confirm ? (
        ...
        <form onSubmit={submit} className="mt-8 space-y-4">
          ...
          <button type="submit" disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 py-3.5 text-[15px] font-bold text-white disabled:opacity-60">
            ...
            {isSignup ? "Create investor account" : "Sign in"}
          </button>
        </form>
      )}
